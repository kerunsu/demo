...
本文是本仓库当前状态的协作入口。它描述“现在如何工作”，不是重构计划，也不替代接口和数据格式的事实文档。历史阶段记录保留在 `docs/archive/` 和 `docs/refactor/`，只用于追溯；如果与代码、测试或本文冲突，以代码和测试为准，并在本文中补充确认后的结论。改动需要找到根源问题并解决而不是单纯补丁。
...
| `app/robot/`、`robot_runtime/`、`doll/` | 机器人控制、Runtime 和动作/表情资源 | Runtime 协议、MP4、DollSer/OSC 适配 |
| `config/`、`doll/data/` | 可审阅的课程、交互、语音和机器人配置 | YAML/JSON/CSV；配置变更必须说明兼容性 |
...
- 一个训练会话内切换课点只追加时间线，不重启连续录制。重复请求、取消、忙碌、断线重连和 Runtime 晚到补传必须保持幂等或明确失败。
- 修改课程交互、事件名、动作/表情/语音绑定或发布规则时，必须同步 `docs/CONTRACT.md`、对应 fixture 和相关前端/Runtime 消费方。
...
当前规范只写在根目录 `AGENTS.md` 和 `docs/` 的事实文档中：`CONTRACT.md`、`DATA_SCHEMA.md`、`TESTING.md`、`CONFIGURATION.md`、`OPERATIONS.md` 及对应专题。`docs/refactor/` 是测试引用的契约快照/验收证据和历史过程资料，不是新的工作指令；新增规则不要写入其中。
