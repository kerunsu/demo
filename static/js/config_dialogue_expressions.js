(() => {
  ...
  const TIERS = [
    { key: 'short', label: '短回复', fallbackMax: 8 },
    { key: 'medium', label: '中回复', fallbackMax: 20 },
    { key: 'long', label: '长回复', fallbackMax: 1000 },
  ];
  ...
  function addRuleRow(rule = {}, tierDefinition) {
    const container = byId('dialogue-expression-rules');
    if (!container) return;
    const row = document.createElement('div');
    row.className = 'dialogue-expression-rule';

    row.dataset.tier = tierDefinition.key;

    const maxLabel = document.createElement('label');
    maxLabel.textContent = tierDefinition.key === 'long'
      ? `${tierDefinition.label}（超过中档）`
      : `${tierDefinition.label}字数上限`;
    ...
    maxInput.value = String(rule.maxChars || tierDefinition.fallbackMax);
    if (tierDefinition.key === 'long') {
      maxInput.value = '1000';
      maxInput.readOnly = true;
      maxInput.setAttribute('aria-label', '长回复兜底上限 1000 字');
    }
    ...
    tierTag.className = 'cc-tiny';
    tierTag.textContent = tierDefinition.key === 'long' ? '兜底档' : '自动匹配';
    row.append(maxLabel, emotionLabel, tierTag);
  ...
  function render() {
    ...
    const container = byId('dialogue-expression-rules');
    container.innerHTML = '';
    const configured = Array.isArray(state.config.rules) ? state.config.rules : [];
    TIERS.forEach((tier, index) => {
      addRuleRow(configured[index] || { tier: tier.key, maxChars: tier.fallbackMax }, tier);
    });
    updatePreview();
  }
  ...
  function updatePreview() {
    ...
    const tier = TIERS.find((item) => item.key === selected.tier);
    target.textContent = `有效字数 ${count}；匹配${tier?.label || ''} ${selected.emotion}`;
  }
