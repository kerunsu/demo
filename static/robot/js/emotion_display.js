...
function playVideoWithoutBlackFrame(eventData, emotionName, style, generation) {
    ...
    incoming.style.transition = 'none';
    incoming.style.opacity = '0';
    incoming.loop = eventData.dialogueReplyLoop === true;
...
function connectWebSocket() {
    ...
    socket.on('dialogue_reply_speech_ended', (data) => {
        if (
            !activeEmotionEvent ||
            activeEmotionEvent.dialogueReply !== true ||
            !matchesExactEmotionEnvelope(activeEmotionEvent, data)
        ) return;
        if (activeMediaElement && 'loop' in activeMediaElement) {
            activeMediaElement.loop = false;
        }
        onEmotionPlayComplete(
            data?.status === 'error' ? 'error' : 'ended',
            data?.reason || 'browser_tts_ended',
        );
    });
}
...
function playEmotion(payload) {
    ...
    console.log(
        eventData.dialogueReplyLoop === true
            ? '[Emotion Display] 对话表情循环到语音结束'
            : '[Emotion Display] 行为表情单次播放，结束后平滑切换下一表情'
    );
