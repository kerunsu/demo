...
本文描述独立 Demo 仓库的现行配置。机械动作和 Robot Runtime 不是可配置选项；屏幕表情、儿童屏鼓励动画均是 Demo 正式能力。
...
| 文件 | 作用 |
|---|---|
| `config/demo_course_scope.json` | 唯一允许课型：`naming`、`ordering` |
| `config/demo_deployment.json` | 机械动作、Robot Runtime 固定关闭；屏幕表情、儿童动画和浏览器语音开启 |
| `config/runtime_modes.yaml` | 儿童媒体固定 `browser`、机器人控制固定 `disabled`，以及浏览器语速/唤醒词 |
| `config/course_presets.json` | 评估与干预两套独立两课程预设 |
| `config/report_scoring.yaml` | 注意力、表达性语言、接收性语言、排序四个报告维度及权重 |
| `config/analyzers.yaml` | 注意力和分析器参数；旧姿态项仅作历史兼容 |
| `doll/data/course_map.json` | 儿童屏动画、屏幕表情与音频时间偏移；不得出现 motion |
...
Demo 运行时只消费命名、排序以及全局注意力/奖励话术。话术写接口会拒绝模仿、配对、拟声、社交等旧课型；旧文件音频条目接口固定返回 410。命名、排序的提示与反馈池变更时要同步互动页面和自动化测试。

## 注意力分析

`config/analyzers.yaml` 和示例文件共同记录可审查默认值。命名、排序训练期间继续采集注意力证据；历史模仿配置只作旧数据兼容，不属于活动课程范围。

配置中心算法页可编辑允许的分析器和报告参数。保存采用同目录临时文件、`fsync` 和原子替换；非法权重、阈值或 schema 返回明确错误，不部分写入。

儿童情绪/注意力观测属于分析数据，可进入监控和报告 KPI；它与屏幕表情输出是两套独立数据。
...
跨课程注意力、表达性语言、接收性语言、排序维度默认权重为 25/25/25/25，总和必须为 100。课程权重只含 `naming`、`ordering`。报告计算、叙事、教师端展示和 Server 报告编辑器必须保持同一范围。
...
导出明确排除数据库、学生个人数据、录制、报告、日志、环境密钥、机械动作和 Robot Runtime；包含 `emotions_meta.json` 和 `static/resources/Emotions/`。导入或从完整版本同步前，必须按 `docs/DEMO_SYNC.md` 人工复核边界。

## 数据库与环境变量

`database/app.db`、`.env`、录制和日志是部署数据，不是源码。首次没有数据库时标准播种创建两课程；已有数据库只原地迁移，不重置、不删除历史行。

常用环境变量（例如端口、对话 provider、是否自动构建教师端）继续按主项目兼容。以下变量在 Demo 不应作为部署配置使用：Robot Runtime 地址/密钥、DollSer/OSC 端口、机械动作模式。即使遗留环境中存在，Demo 能力层仍固定关闭这些机械能力。
