...
Demo 发布必须同时证明两课程正常、硬件能力永久关闭、全新拉取可复现。单独通过主流程测试不足以发布。
...
- 有效课型精确等于 `pairing/ordering`，非法范围文件 fail-closed。
- 静态课程目录、课程预设、首次数据库和配置 API 只暴露两类课程。
- 报告输出跨课程注意力、配对、排序三个维度，课程投影只含配对和排序。
...
- 注意力：配对/排序期间持续采集、缺失设备时明确降级、历史数据只读兼容。
