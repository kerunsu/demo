...
def launch_swap_and_exit(prepared: Dict[str, Any]) -> Dict[str, Any]:
    ...
    lines.extend([
        ...
        "  Write-UpdateLog 'new Runtime is online; restoring robot pages'",
        "  try {",
        "    Invoke-RestMethod -Method Post -Uri ($RuntimeUrl + '/ui/open-emotion') -ContentType 'application/json' -Body '{}' -TimeoutSec 10 | Out-Null",
        "  } catch { Write-UpdateLog ('emotion page restore warning: ' + $_.Exception.Message) }",
        "  try {",
