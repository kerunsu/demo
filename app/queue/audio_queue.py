...
logger = setup_logger('audio_queue')
...
class AudioQueue:
    ...
    def __init__(self, max_size: int = None):
        ...
        Args:
            max_size: 每个会话队列的最大大小（默认使用Config.AUDIO_QUEUE_SIZE）
        """
        self.max_size = max_size or Config.AUDIO_QUEUE_SIZE
        # 存储每个会话的队列：{session_id: deque}
        self._queues: dict[str, deque] = {}
        # 锁，保证线程安全
        self._lock = threading.RLock()
        
        logger.debug(f"初始化音频队列: max_size={self.max_size}")
        ...
        try:
            with self._lock:
                ...
                if session_id not in self._queues:
                    ...
                    logger.debug(f"为会话创建音频队列: session_id={session_id}")
                
                queue = self._queues[session_id]
                
                # 如果队列已满，最旧的块会被自动丢弃（deque的maxlen特性）
                queue.append((chunk, timestamp, session_id))
            ...
            logger.error(f"放入音频块到队列失败: {e}", exc_info=True)
            return False
    ...
    def get(self, session_id: str) -> Optional[Tuple[Any, float, str]]:
        ...
        Returns:
            (chunk, timestamp, session_id) 元组，如果队列为空则返回None
        """
        if not session_id:
        ...
        try:
            ...
            logger.error(f"从音频队列获取块失败: {e}", exc_info=True)
            return None
        ...
        try:
            with self._lock:
                if session_id in self._queues:
                    ...
                    logger.debug(f"清空音频队列: session_id={session_id}")
                    return True
                return False
                
        except Exception as e:
            logger.error(f"清空音频队列失败: {e}", exc_info=True)
            return False
        ...
        try:
            with self._lock:
                if session_id in self._queues:
                    ...
                    logger.debug(f"移除音频队列: session_id={session_id}")
                    return True
                return False
                
        except Exception as e:
            logger.error(f"移除音频队列失败: {e}", exc_info=True)
            return False
    ...
    def size(self, session_id: Optional[str] = None) -> int:
        ...
        try:
            ...
            logger.error(f"获取音频队列大小失败: {e}", exc_info=True)
            return 0
