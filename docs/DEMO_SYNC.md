...
| 能力 | Demo 规则 | 事实源 |
|---|---|---|
| 课程 | 只启用模仿识别、配对、排序 | `config/demo_course_scope.json` |
| 分析与报告 | 只统计和展示上述三类课程 | 同上；`app/report/` |
...
通常可以同步：通用缺陷修复、采集和存储兼容、模仿识别、配对/排序交互、浏览器语音、儿童页面、教师端通用体验、报告审核、数据库兼容迁移和安全修复。
...
2. 对比最近更新，按文件和功能挑选与三门课程或通用平台相关的变更。
3. 先同步事实源和数据迁移，再同步后端处理，最后同步教师端/儿童端。
4. 重新核对两个 Demo JSON 事实源、固定 `disabled` 运行模式、三课程预设和最小 `course_map.json`。
...
引导脚本会补齐依赖、安装教师端依赖、在缺少 `database/app.db` 时运行 `database/seed_standard.py`，并创建仅含模仿、配对和排序的标准库。数据库、录制、日志和 `node_modules` 不进入 Git；课程目录、模型、浏览器交互页面、语音配置和儿童动画必须随仓库提交。
...
- `/courses`、课程预设、配置目录和新报告只含 `mimic/pairing/ordering`。
...
- 配对/排序完整反馈后进入评分，模仿识别在 Windows 使用仓库内模型，三类课程均可生成并审核报告。
